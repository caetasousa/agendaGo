...
test('cadastro como prestador exige confirmação por email antes de logar', async ({
	page,
	request
}) => {
	const email = emailUnico('prestador');
...
test('cadastro de prestador com email já em uso responde igual, sem revelar nada', async ({
	page
}) => {
	const email = emailUnico('prestador-duplicado');

	async function preencherEEnviar() {
		await page.goto('/cadastro?tipo=prestador');
		await page.fill('#nome', 'Prestador Duplicado');
		await page.fill('#email', email);
		await page.fill('#telefone', '(11) 99999-8888');
		await page.fill('#senha', '12345678');
		await page.fill('#confirmar-senha', '12345678');
	...
	// enviar de novo com o mesmo email (agora com um pendente já em aberto)
	// responde exatamente igual — nada na tela denuncia a diferença
	await preencherEEnviar();
	await expect(page.getByText(`Enviamos um email para ${email}`)).toBeVisible();
});

test('cadastro como cliente exige confirmação por email antes de logar', async ({
	page,
	request
}) => {
