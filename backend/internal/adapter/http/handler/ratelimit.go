...
// projeto (um container da API atrás do Caddy). Com mais de uma réplica, cada
// uma teria o seu contador e o teto efetivo seria multiplicado pelo número de
// réplicas; nesse dia, um contador compartilhado (httprate.WithLimitCounter).
...
// contabiliza a tentativa atual — isso é papel de RegistrarFalha. Falha do
// contador nunca barra a requisição: um erro de infraestrutura não pode virar
// porta fechada para quem tem a senha certa.
...
//
// No login, só o fracasso é registrado. Na recuperação de senha, toda
// solicitação conta: a resposta é sempre a mesma (204, para não revelar quais
// emails existem), então não há sucesso ou fracasso a distinguir — e o que se
// quer limitar ali é justamente o volume de emails disparados para uma pessoa.
