...
func NovoRedefinirSenhaUseCase(usuarios contaUsuario, clients contaClient, resets repositorioResetSenha, sessoes repositorioSessao, hasher hasherSenha) *RedefinirSenhaUseCase {
	return &RedefinirSenhaUseCase{usuarios: usuarios, clients: clients, resets: resets, sessoes: sessoes, hasher: hasher}
}
...
func (uc *RedefinirSenhaUseCase) Executar(tokenPuro, novaSenha string) error {
	...
	case session.TipoProvider:
		if err := uc.usuarios.AtualizarSenha(reset.UserID, hash); err != nil {
			return err
		}
